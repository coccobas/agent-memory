...
function getRulesSourceDir(): string {
  ...
  return join(projectRoot, 'rules', 'rules');
}
...
function concatenateForClaude(files: Map<string, string>): string {
  ...
  // Sort files to ensure consistent order (core first)
  const sortedFiles = [...files.entries()].sort((a, b) => {
    if (a[0].includes('core')) return -1;
    if (b[0].includes('core')) return 1;
    return a[0].localeCompare(b[0]);
  });

  for (const [filename, content] of sortedFiles) {
    // Remove frontmatter for Claude (it doesn't use it)
