  ...
  const nodeColumns: ColumnDef<GraphNode, unknown>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'Type',
        cell: ({ row }) => <Badge variant="secondary">{row.getValue('nodeTypeName')}</Badge>,
      },
      {
        accessorKey: 'id',
        header: 'ID',
    ...
    () => [
      {
        ...
        header: 'Relation',
        cell: ({ row }) => <Badge variant="default">{row.getValue('edgeTypeName')}</Badge>,
      },
      {
    ...
    <div className="space-y-4">
      ...
      {viewMode === 'graph' ? (
        <div ref={containerRef} className="relative">
          ...
          {selectedNode && (
            <div
              className="absolute top-0 right-0 bg-card border border-border rounded-lg overflow-hidden shadow-lg"
