...
#
# Purpose:
# - Record tool execution outcomes (success/failure/partial)
# - Track tool sequences and durations for learning
# - Non-blocking (runs after tool execution completes)
...
exec agent-memory hook posttooluse${projectId ? ` --project-id "${projectId}"` : ''} --agent-id "\${AGENT_ID}"
`;
}
...
export function generateClaudeCodeSettings(projectPath: string): string {
  ...
  const postToolUsePath = path.join(projectPath, '.claude', 'hooks', 'posttooluse.sh');
  ...
  try {
    switch (config.ide) {
      case 'claude': {
        ...
        // PostToolUse (outcome recording) hook script
        hooks.push({
          filePath: path.join(config.projectPath, '.claude', 'hooks', 'posttooluse.sh'),
          content: generateClaudeCodePostToolUseHookScript({ projectId: config.projectId }),
          instructions: generateClaudeCodeInstructions(config.projectPath),
        });
