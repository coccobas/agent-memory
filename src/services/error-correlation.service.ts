 ...
 */
export function calculateErrorCorrelation(params: ErrorCorrelationParams): ErrorCorrelationResult {
 ...
 */
export function detectLowDiversity(projectId: string): LowDiversityResult {
  const db = getDb();

  // Get all unique agents that have errors in this project
  const agentErrors = db
    .select({
    ...
    .groupBy(auditLog.agentId)
    .all();

  const agentIds = agentErrors.map((e) => e.agentId).filter((id): id is string => id !== null);
