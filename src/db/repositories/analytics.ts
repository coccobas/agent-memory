 ...
 *
 * Handles analytical queries on the audit log for usage statistics,
 * trends, and subtask analytics.
 */

import { eq, and, gte, lte, sql, desc, count, isNotNull, type SQL } from 'drizzle-orm';
import type { DrizzleDb } from './base.js';
import { auditLog, entryTags, tags } from '../schema.js';
import type { ScopeType, EntryType } from '../schema.js';
import type {
 ...
 */
export function createAnalyticsRepository(db: DrizzleDb): IAnalyticsRepository {
  return {
    /**
     ...
     */
    async getTrends(params: UsageStatsParams = {}): Promise<TrendData[]> {
      const { scopeType, scopeId, startDate, endDate } = params;
     ...
     */
    async getSubtaskStats(params: SubtaskStatsParams = {}): Promise<SubtaskStats> {
      // Build where conditions
      const conditions: SQL<unknown>[] = [isNotNull(auditLog.subtaskType)];
