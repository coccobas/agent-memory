...
interface SessionStartOptions extends Record<string, unknown> {
  projectId?: string;
  name?: string;
  purpose?: string;
}

interface SessionListOptions extends Record<string, unknown> {
  projectId?: string;
  status?: string;
  limit?: number;
  offset?: number;
...
interface SessionEndOptions extends Record<string, unknown> {
  id: string;
  status?: string;
}
...
export function addSessionCommand(program: Command): void {
  ...
  session
    ...
    .action(
      typedAction<SessionStartOptions>(async (options, globalOpts) => {
        try {
  ...
  session
    ...
    .action(
      typedAction<SessionListOptions>(async (options, globalOpts) => {
        try {
          const context = await getCliContext();
  ...
  session
    ...
    .action(
      typedAction<SessionEndOptions>(async (options, globalOpts) => {
        try {
          const context = await getCliContext();
