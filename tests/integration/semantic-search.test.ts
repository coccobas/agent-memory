...
describe('Semantic Search Integration', () => {
  ...
  it.skipIf(() => !embeddingService?.isAvailable())('should generate and store embeddings for tools', async () => {
    ...
    const embeddingRecord = db
      ...
      .where(
        and(
          ...
          eq(entryEmbeddings.entryId, tool.id)
        )
      )
      .get();
  ...
  it.skipIf(() => !embeddingService?.isAvailable())('should find semantically similar tools', async () => {
    ...
    // Generate embeddings
    await generateEmbeddingAsync('tool', loginTool.id, loginVersion.id, loginTool.name, loginVersion);
  ...
  it.skipIf(() => !embeddingService?.isAvailable())('should generate and store embeddings for guidelines', async () => {
  ...
  it.skipIf(() => !embeddingService?.isAvailable())('should find semantically similar guidelines', async () => {
    ...
    // Generate embeddings
    await generateEmbeddingAsync('guideline', sqlGuideline.id, sqlVersion.id, sqlGuideline.name, sqlVersion);
    await generateEmbeddingAsync('guideline', xssGuideline.id, xssVersion.id, xssGuideline.name, xssVersion);
  ...
  it.skipIf(() => !embeddingService?.isAvailable())('should generate and store embeddings for knowledge entries', async () => {
  ...
  it.skipIf(() => !embeddingService?.isAvailable())('should verify hybrid scoring with semantic similarity', async () => {
  ...
  it.skipIf(() => !embeddingService?.isAvailable())('should properly sort results by semantic similarity', async () => {
  ...
  it.skipIf(() => !embeddingService?.isAvailable())('should handle different semantic thresholds', async () => {
    // Create a tool
    const { tool, version } = createTestTool(
      db,
      'file-processor',
  ...
  it.skipIf(() => !embeddingService?.isAvailable())('should search across multiple entry types', async () => {
    ...
    await generateEmbeddingAsync('guideline', guideline.id, guidelineVersion.id, guideline.name, guidelineVersion);
